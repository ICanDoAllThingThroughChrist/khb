class CustomerSearchTerm 
    ...
    def build_for_email_search(search_term)  
        @where_clause << case_insensitivie_search(:first_name)
        @where_args[:first_name] = starts_with(extract_name(search_term)) 
        @where_clause << "OR #{case_insensitivie_search(:last_name)}"
        @where_args[:last_name] = starts_with(extract_name(search_term)) 
        @where_clause << "OR #{case_insensitivie_search(:email)}"
...
def case_insensitivie_search(field_name)
